...
export function PageHero({
  ...
  const resolvedPrimaryCta = primaryCta ?? {
    label: tc('scheduleCall'),
    href: site.calendarUrl,
  };
  const hasSideMedia = !videoSrc && Boolean(image || visual);
  const hasMedia = Boolean(videoSrc || image || visual);
  ...
  return (
    <section className="relative overflow-hidden bg-ink-950 pt-36 pb-20 sm:pt-44 sm:pb-24">
      ...
      <div className="container-x relative z-[1]">
        ...
        >
          <div className={hasSideMedia ? 'min-w-0 max-w-[21rem] sm:max-w-none' : 'max-w-3xl'}>
            {eyebrow && (
              <Reveal>
            ...
            {price && (
              <Reveal delay={150}>
                <div className="mt-9 inline-flex items-end gap-3 bg-surface px-6 py-4 backdrop-blur-sm">
                  {price.before && (
                    <span className="text-lg text-faint line-through">{price.before}</span>
                  )}
                  <span className="font-display text-3xl font-semibold text-accent">
                    {price.now}
                  </span>
                  {price.note && (
                    <span className="pb-1 font-mono text-sm text-faint">{price.note}</span>
                  )}
                </div>
              </Reveal>
            )}
